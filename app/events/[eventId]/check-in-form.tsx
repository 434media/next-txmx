...
export default function CheckInForm({ eventId }: CheckInFormProps) {
  const { user } = useAuth()
  ...
  // Check the user's current check-in state on mount
  useEffect(() => {
    if (!user) {
      setChecking(false)
      return
    ...
    let cancelled = false
    isUserCheckedIn(user.uid, eventId)
      .then((flag) => {
        if (!cancelled) {
          setCheckedIn(flag)
          setChecking(false)
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    setError("")
    try {
      const res = await submitCheckInCode(user.uid, eventId, cleaned)
      if (res.success) {
        setCheckedIn(true)
        setCode("")
      } else {
        setError(res.error || "Code is incorrect or expired")
        // Clear and refocus the input on error
        setCode("")
  ...
  return (
    <div className="border border-amber-500/30 rounded-xl bg-amber-500/5 px-5 py-5">
      ...
      <p className="text-white/50 text-xs font-medium leading-5 mb-4">
        Only checked-in fans are eligible for the BOXR Station prize. Codes
        rotate during the night.
      </p>

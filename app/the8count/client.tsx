...
interface The8CountClientProps {
  feedItems: FeedItem[]
  feedTypes: { id: string; label: string; count: number }[]
  feedTopics: { id: string; label: string; count: number }[]
}

export function The8CountClient({ feedItems, feedTypes, feedTopics }: The8CountClientProps) {
  ...
  const handleTopicToggle = (topic: string) => {
    setSelectedTopics((prev: string[]) => (prev.includes(topic) ? prev.filter((t: string) => t !== topic) : [...prev, topic]))
  }
        ...
        dangerouslySetInnerHTML={{
          __html: JSON.stringify({
            ...
            blogPost: feedItems.slice(0, 10).map((item) => ({
              ...
              author: item.authors.map((author: string) => ({
                "@type": "Person",
                name: author,
              })),

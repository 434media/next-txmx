"use client"

import { useState } from "react"
import { useAuth } from "../../lib/auth-context"

/**
 * Shared sign-up / sign-in form used by the hub's "Join the Game" band and the
 * slug page's "Get Started" section so the two stay identical. Email + password
 * is the primary path; Google is the secondary option below. Self-contained —
 * owns its auth state and calls useAuth directly.
 */
export default function SignUpForm() {
  const { signInWithGoogle, signUpWithEmail, signInWithEmail } = useAuth()
  const [mode, setMode] = useState<"signup" | "signin">("signup")
  const [name, setName] = useState("")
  const [email, setEmail] = useState("")
  const [password, setPassword] = useState("")
  const [busy, setBusy] = useState(false)
  const [error, setError] = useState("")

  const handleGoogle = async () => {
    setBusy(true)
    setError("")
    try {
      await signInWithGoogle()
    } catch (e) {
      if (e instanceof Error && !e.message.includes("popup-closed")) {
        setError("Google sign-in failed. Try again.")
      }
    } finally {
      setBusy(false)
    }
  }

  const handleEmail = async (e: React.FormEvent) => {
    e.preventDefault()
    setBusy(true)
    setError("")
    try {
      if (mode === "signup") await signUpWithEmail(email, password, name)
      else await signInWithEmail(email, password)
    } catch {
      setError(
        mode === "signup"
          ? "Couldn't create that account. The email may already be in use."
          : "Invalid email or password."
      )
    } finally {
      setBusy(false)
    }
  }

  const isSignup = mode === "signup"

  return (
    <>
      {error && (
        <div className="mb-4 rounded-lg border border-red-200 bg-red-50 px-4 py-3">
          <p className="text-red-600 text-xs font-medium">{error}</p>
        </div>
      )}

      {/* Email + password — the primary path. Display name (signup only) is
          collected so the leaderboard shows a real name, not "Fan". */}
      <form onSubmit={handleEmail} className="space-y-3">
        {isSignup && (
          <input
            type="text"
            placeholder="Display name"
            value={name}
            onChange={(e) => setName(e.target.value)}
            required
            autoComplete="name"
            maxLength={40}
            className="w-full px-4 py-3 rounded-md bg-white border border-neutral-300 text-sm text-neutral-900 placeholder-neutral-400 focus:outline-none focus:border-amber-500 transition-colors"
          />
        )}
        <input
          type="email"
          placeholder="Email"
          value={email}
          onChange={(e) => setEmail(e.target.value)}
          required
          autoComplete="email"
          className="w-full px-4 py-3 rounded-md bg-white border border-neutral-300 text-sm text-neutral-900 placeholder-neutral-400 focus:outline-none focus:border-amber-500 transition-colors"
        />
        <input
          type="password"
          placeholder="Password"
          value={password}
          onChange={(e) => setPassword(e.target.value)}
          required
          minLength={6}
          autoComplete={isSignup ? "new-password" : "current-password"}
          className="w-full px-4 py-3 rounded-md bg-white border border-neutral-300 text-sm text-neutral-900 placeholder-neutral-400 focus:outline-none focus:border-amber-500 transition-colors"
        />
        <button
          type="submit"
          disabled={busy}
          className="w-full inline-flex items-center justify-center px-4 py-3 bg-amber-500 text-black text-xs font-bold tracking-[0.2em] uppercase hover:bg-amber-400 transition-colors rounded-md disabled:opacity-50"
        >
          {busy ? "Working…" : isSignup ? "Create account" : "Sign in"}
        </button>
      </form>

      {/* Divider */}
      <div className="flex items-center gap-3 my-4">
        <span className="h-px flex-1 bg-neutral-200" />
        <span className="text-neutral-400 text-[10px] font-bold tracking-[0.2em] uppercase">
          or
        </span>
        <span className="h-px flex-1 bg-neutral-200" />
      </div>

      {/* Google — the secondary option, below */}
      <button
        onClick={handleGoogle}
        disabled={busy}
        className="w-full inline-flex items-center justify-center gap-2 px-4 py-3 bg-white border border-neutral-300 text-neutral-900 text-xs font-bold tracking-[0.2em] uppercase hover:bg-neutral-100 transition-colors rounded-md disabled:opacity-50"
      >
        <GoogleIcon className="w-4 h-4" />
        Continue with Google
      </button>

      {/* Mode toggle */}
      <p className="text-neutral-500 text-xs mt-5">
        {isSignup ? "Already have an account?" : "Need an account?"}{" "}
        <button
          type="button"
          onClick={() => {
            setMode((m) => (m === "signup" ? "signin" : "signup"))
            setError("")
          }}
          className="text-amber-600 font-semibold hover:text-amber-700 transition-colors"
        >
          {isSignup ? "Sign in" : "Create one"}
        </button>
      </p>
    </>
  )
}

function GoogleIcon({ className = "" }: { className?: string }) {
  return (
    <svg className={className} viewBox="0 0 24 24" aria-hidden>
      <path
        fill="#4285F4"
        d="M22.56 12.25c0-.78-.07-1.53-.2-2.25H12v4.26h5.92a5.06 5.06 0 01-2.2 3.32v2.77h3.57c2.08-1.92 3.28-4.74 3.28-8.1z"
      />
      <path
        fill="#34A853"
        d="M12 23c2.97 0 5.46-.98 7.28-2.66l-3.57-2.77c-.98.66-2.23 1.06-3.71 1.06-2.86 0-5.29-1.93-6.16-4.53H2.18v2.84C3.99 20.53 7.7 23 12 23z"
      />
      <path
        fill="#FBBC05"
        d="M5.84 14.09c-.22-.66-.35-1.36-.35-2.09s.13-1.43.35-2.09V7.07H2.18C1.43 8.55 1 10.22 1 12s.43 3.45 1.18 4.93l2.85-2.22.81-.62z"
      />
      <path
        fill="#EA4335"
        d="M12 5.38c1.62 0 3.06.56 4.21 1.64l3.15-3.15C17.45 2.09 14.97 1 12 1 7.7 1 3.99 3.47 2.18 7.07l3.66 2.84c.87-2.6 3.3-4.53 6.16-4.53z"
      />
    </svg>
  )
}
